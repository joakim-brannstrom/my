 ...
 */
private struct ActorControlBlock(T) {
    T item;
    /// Number of RefCounted instances.
    shared int strongCnt = 1;
    /// Number of weak references. +1 if strongCnt isn't zero.
...
private void incrStrongCnt(T)(ref T cb) nothrow {
    cb.strongCnt.atomicOp!"+="(1);
}

private void releaseUseCnt(T)(ref T cb) {
    assert(cb.strongCnt >= 0, "Invalid count detected");

    if (cb.strongCnt.atomicOp!"-="(1) == 0) {
        destroy(cb.item);
        releaseWeakCnt(cb);
    }
}
 ...
 */
struct RefCounted(T) {
    alias Impl = ActorControlBlock!T;
    private Impl* impl;
    private T* item;
    ...
    int refCount() @safe pure nothrow const @nogc {
        if (impl) {
            return atomicLoad(impl.strongCnt);
        }
        return 0;
    }
...
struct WeakRef(T) {
    ...
    RefCounted!T asRefCounted() nothrow {
        ...
        auto strongCnt = atomicLoad(impl.strongCnt);
        if (strongCnt == 0)
            return RefCounted!T.init;

        cas(&impl.strongCnt, strongCnt, strongCnt + 1);
...
@safe unittest {
...

    {
        ...
        assert(rc1.impl.strongCnt == 1);
        assert(rc1.impl.weakCnt == 2);
        assert(rc2.impl.strongCnt == 1);
